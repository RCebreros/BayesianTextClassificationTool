...
class Optimizer:
    ...
    def gradientDescent(self, X: pd.DataFrame[int, np.ndarray], y: pd.DataFrame[int, np.ndarray], **kwargs):
        """Linear Regression optimization function implementing gradient descent (using MSE).
        ...
        for _ in range(self.epochs):
            gradient_w, gradient_b = self._computeGradients(X, y, n_features, weights, bias)
    ...
    # ========================================================================================== COMPUTE GRADIENTS
    @staticmethod
    def _computeGradients(X, y, n_features, weights, bias):
